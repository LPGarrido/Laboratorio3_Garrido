# 1 "main_laboratorio3.s"
# 1 "<built-in>" 1
# 1 "main_laboratorio3.s" 2
;Archivo: main_prelab3.s
;Dispositivo: PIC16F887
;Autor: Luis Garrido
;Compilador: pic-as (v2.30), MPLABX V5.40
;
;Programa: contador de 4 bits en el puerto D
;Hardware: leds en el puerto D con su respectiva resistencia
;Creado: 06 feb, 2022
;Última modificación: 06 feb, 2022
...
LOOP:
    BTFSC ((INTCON) and 07Fh), 2 ; ((INTCON) and 07Fh), 2 = 1, Evaluar ; ((INTCON) and 07Fh), 2=1, CALL FUNC_TMR0
    CALL FUNC_TMR0
    BTFSS PORTA,0 ; ((PORTA) and 07Fh), 0 = 0,CALL INC_CONT; ((PORTA) and 07Fh), 0=1, Evaluar
    CALL INC_CONT
    BTFSS PORTA,1 ; ((PORTA) and 07Fh), 1 = 0,CALL DEC_CONT; ((PORTA) and 07Fh), 1=1, Regresa
    CALL DEC_CONT
    CALL COMPARADOR
    GOTO LOOP
...
;LOOP:
; BTFSS ((INTCON) and 07Fh), 2 ; Interrupción del TMR0 - ((INTCON) and 07Fh), 2=0, GOTO LOOP; ((INTCON) and 07Fh), 2=1, CALL RESET_TMR0
; GOTO LOOP ; Regresa a evaluar
; ; Cuando ((INTCON) and 07Fh), 2 = 1, se resetea el TMR0 y ejecuta las instrucciones.
; CALL RESET_TMR0 ; Resetear el TMR0
; INCF PORTD,F ; Incrementar en 1 el PORTD
; GOTO LOOP ; Regresa a evaluar la interrupción del TMR0
...
CONFIG_IO:
    ...
    MOVLW 0x03 ; Pasar el num a W
    MOVWF TRISA ; Pasar W al TRISA (((PORTA) and 07Fh), 0,((PORTA) and 07Fh), 1 como Entradas)
    MOVLW 0xF0 ; Pasar el num a W
    MOVWF TRISD ; Pasar W al TRISD (((PORTD) and 07Fh), 0,((PORTD) and 07Fh), 1,((PORTD) and 07Fh), 2,((PORTD) and 07Fh), 3 como Salida)
    CLRF TRISC ; Limpiar TRISC (PORTC como Salidas)
    MOVLW 0xF0 ; Pasar el num a W
    MOVWF TRISB ; Pasar W al TRISD (((PORTD) and 07Fh), 0,((PORTD) and 07Fh), 1,((PORTD) and 07Fh), 2,((PORTD) and 07Fh), 3 como Salida)
    BCF TRISE,0
    BANKSEL PORTD ; Cambio de banco
...
CONFIG_RELOJ:
    ...
    BSF ((OSCCON) and 07Fh), 0 ; ((OSCCON) and 07Fh), 0 = 1, Usamos reloj interno
    BSF ((OSCCON) and 07Fh), 6 ; ((OSCCON) and 07Fh), 6 = 1
    BCF ((OSCCON) and 07Fh), 5 ; ((OSCCON) and 07Fh), 5 = 0
    BSF ((OSCCON) and 07Fh), 4 ; ((OSCCON) and 07Fh), 4 = 1 - IRCF<2:0> => 101 2MHz
    RETURN ; Regresar

; Configuramos el TMR0 para obtener un retardo de 100ms
CONFIG_TMR0:
    BANKSEL OPTION_REG ; Cambio de banco
    BCF ((OPTION_REG) and 07Fh), 5 ; TMR0 = 0, Timer0 como temporizador
    BCF ((OPTION_REG) and 07Fh), 3 ; Prescaler a TMR0
    BSF ((OPTION_REG) and 07Fh), 2 ; ((OPTION_REG) and 07Fh), 2 = 1
    BSF ((OPTION_REG) and 07Fh), 1 ; ((OPTION_REG) and 07Fh), 1 = 1
    BSF ((OPTION_REG) and 07Fh), 0 ; ((OPTION_REG) and 07Fh), 0 = 1 - PS<2:0> => 111 prescaler 1 : 256
...
INC_CONT:
    BTFSS PORTA,0 ;((PORTA) and 07Fh), 0=0 - GOTO INC_CONT, ((PORTA) and 07Fh), 0=1 - INCREMENTAR
    GOTO INC_CONT
; GOTO $+2 ;Incrementa
; RETURN ;Retornar
...
DEC_CONT:
    BTFSS PORTA,1 ;((PORTA) and 07Fh), 1=0 - GOTO CONT, ((PORTA) and 07Fh), 1=1 - DECF CONT,F
...
FUNC_TMR0:
    ...
    CALL RESET_TMR0 ; Resetear el TMR0
    INCF PORTD,F ; Incrementar en 1 el PORTD
    RETURN ; Regresa a evaluar la interrupción del TMR0
