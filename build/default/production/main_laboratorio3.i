...
LOOP:
    BTFSC ((INTCON) and 07Fh), 2 ; ((INTCON) and 07Fh), 2 = 0, Evaluar ; ((INTCON) and 07Fh), 2=1, CALL FUNC_TMR0
    CALL FUNC_TMR0
    BTFSS PORTA,0 ; ((PORTA) and 07Fh), 0 = 0,CALL INC_CONT; ((PORTA) and 07Fh), 0=1, Evaluar
    CALL INC_CONT
    BTFSS PORTA,1 ; ((PORTA) and 07Fh), 1 = 0,CALL DEC_CONT; ((PORTA) and 07Fh), 1=1, Evaluar
    CALL DEC_CONT
...
COMPARADOR:
    ...
    CALL CAMBIO ; CAMBIO del bit ((PORTE) and 07Fh), 0
    RETURN

CAMBIO:
    CLRF PORTB ; Limipiar el PORTB
    INCF PORTE ; Si ((PORTE) and 07Fh), 0=0 pasa a ser 1 y si ((PORTE) and 07Fh), 0=1 pasa a ser 0
    RETURN ; Regresa
